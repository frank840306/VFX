...
% Long description
    ...
    for idx = 1:img_size
        for new_y = 1:img_h
            for new_x = 1:img_w
                x = focal_length(idx) * tan((new_x - mid_x) / focal_length(idx));
                y = (new_y - mid_y) * (sqrt(x * x + focal_length(idx) * focal_length(idx))) / focal_length(idx);
                x = x + mid_x;
                y = y + mid_y;
                % reconstruct
                if (1 <= x) && (x <= img_w) && (1 <= y) && (y <= img_h)
                    ...
                    warpingImages(new_y, new_x, :, idx) = images(round(y), round(x), :, idx);    
                end
            end
        end
    end
